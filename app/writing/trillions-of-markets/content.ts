...
<section>
  ...
  <p>Most of that surface should never trade. Many possible claims are redundant with existing instruments. Many are too small to matter. Many cannot be verified. Many are illegal, manipulable, predatory, or socially destructive. The relevant object is the filtered space: claims that add a residual exposure existing markets do not already provide, whose payoff can be verified, whose risk can be priced or hedged, whose legal wrapper can be enforced, and whose social consequences are admissible.</p>
  <p>The claim is that this filtered space can still be enormous. The future does not price every possible state of the world. It prices the subset whose residual value exceeds the cost of representation.</p>
